...
export default function Profiles() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 px-2 lg:px-0 text-slate-900 dark:text-white">
      ...
      {profiles.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center glass-panel rounded-[2.5rem] bg-white/40 dark:bg-slate-800/40 border border-white/20">
          ...
          <p className="text-slate-500 font-bold max-w-sm mb-8">
            Profiles help you separate personal, business, and shared expenses.
          </p>
          <button
            onClick={() => setCreateOpen(true)}
            className="bg-blue-600 hover:bg-blue-700 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-blue-600/30"
          >
          ...
          {profiles.map((profile, index) => {
            ...
            return (
              <div
                key={profile.id}
                onClick={() =>
                  setDetailProfile({ ...profile, colorIndex: index })
                }
              ...
              >
                ...
                <div className="flex justify-between items-start mb-6">
                  <div
                    className={`w-14 h-14 lg:w-16 lg:h-16 rounded-[1.25rem] lg:rounded-[1.5rem] bg-gradient-to-tr ${GRADIENT_COLORS[index % GRADIENT_COLORS.length]} flex items-center justify-center text-white font-black text-xl lg:text-2xl shadow-xl border-2 border-white/20 overflow-hidden`}
                  >
                    {profile.avatar_url ? (
                      <img
                ...
                <h3 className="text-xl lg:text-2xl font-black text-slate-900 dark:text-white mb-2 truncate group-hover:text-blue-600 transition-colors">
                  {profile.name}
                </h3>
      ...
      {detailProfile && (
        <div
          className="fixed inset-0 z-[150] flex items-center justify-center p-2 lg:p-4 bg-black/70 backdrop-blur-md"
          onClick={() => setDetailProfile(null)}
        >
          <div
            className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] lg:rounded-[3rem] shadow-2xl border border-white/10 overflow-hidden animate-in fade-in zoom-in-95 duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className={`h-32 lg:h-40 bg-gradient-to-br ${GRADIENT_COLORS[detailProfile.colorIndex % GRADIENT_COLORS.length]} relative`}
            >
              <div className="absolute inset-0 bg-black/10" />
              <button
                onClick={() => setDetailProfile(null)}
                className="absolute top-4 right-4 p-2 rounded-xl bg-black/20 hover:bg-black/30 text-white transition-all backdrop-blur-md z-20"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="px-8 lg:px-10 pb-8 lg:pb-10 relative">
              <div className="relative -mt-12 lg:-mt-16 mb-6">
                <div
                  className={`w-24 h-24 lg:w-32 lg:h-32 rounded-[2rem] lg:rounded-[2.5rem] bg-gradient-to-tr ${GRADIENT_COLORS[detailProfile.colorIndex % GRADIENT_COLORS.length]} flex items-center justify-center text-white font-black text-3xl lg:text-5xl shadow-2xl border-8 border-white dark:border-slate-900 overflow-hidden`}
                >
                  {detailProfile.avatar_url ? (
                    <img
                      src={detailProfile.avatar_url}
                      alt="avatar"
                      className="w-full h-full object-cover"
                    />
                  ) : (
              ...
              <div className="mb-8">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <div className="px-3 py-1 bg-blue-500 text-white text-[10px] font-black uppercase rounded-lg tracking-widest">
                    {detailProfile.currency || "USD"}
                  </div>
                  <span className="text-xs text-slate-400 font-bold flex items-center gap-1.5">
                    <Calendar className="w-4 h-4" />{" "}
                    {new Date(detailProfile.created_at).toLocaleDateString()}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4 bg-slate-50 dark:bg-black/20 rounded-3xl p-6 mb-8 border border-black/5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono text-[9px] text-slate-500 bg-white dark:bg-slate-800 px-2 py-1 rounded-lg border border-black/5">
                    {detailProfile.id}
                  </span>
                </div>
              </div>
      ...
      {createOpen && (
        ...
        >
          <div
            className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-white/10 overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            >
              ...
              >
                {creating ? (
                  ...
                  <>
                    <Plus className="w-6 h-6" /> Create Profile
                  </>
                )}
              </button>
            </form>
