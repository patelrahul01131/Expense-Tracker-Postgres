...
function Home() {
      ...
      setStats([
        {
          ...stats[0],
          value: income,
          change: `${percentage_calculate(income, lastIncome).toFixed(2)}%`,
          type:
            percentage_calculate(income, lastIncome) >= 0
              ? "positive"
              : "negative",
        },
        {
          ...stats[1],
          value: expense,
          change: `${percentage_calculate(expense, lastExpense).toFixed(2)}%`,
          type:
            percentage_calculate(expense, lastExpense) <= 0
              ? "positive"
              : "negative",
        },
        {
          ...stats[2],
          value: balance,
          change: `${percentage_calculate(balance, lastBalance).toFixed(2)}%`,
          type:
            percentage_calculate(balance, lastBalance) >= 0
              ? "positive"
              : "negative",
        },
        {
          ...
          change: `${percentage_calculate(BurnRate, lastBurnRate).toFixed(2)}%`,
          type:
            percentage_calculate(BurnRate, lastBurnRate) <= 0
              ? "positive"
              : "negative",
        },
        {
          ...stats[4],
          value: SpendingEfficiency.toFixed(2),
          change: `${percentage_calculate(SpendingEfficiency, lastSpendingEfficiency).toFixed(2)}%`,
          type:
            percentage_calculate(SpendingEfficiency, lastSpendingEfficiency) >=
            0
              ? "positive"
              : "negative",
        },
        {
          ...stats[5],
          value: SavingEfficiency.toFixed(2),
          change: `${percentage_calculate(SavingEfficiency, lastSavingEfficiency).toFixed(2)}%`,
          type:
            percentage_calculate(SavingEfficiency, lastSavingEfficiency) >= 0
              ? "positive"
              : "negative",
        },
      ]);
  ...
  const handleTransaction = async (e) => {
    ...
    try {
      await axios.post("http://localhost:3000/api/expense/add", model_values, {
        headers: getHeaders(),
      });
    ...
    <div className="max-w-7xl mx-auto space-y-4 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 px-2 lg:px-0 text-slate-900 dark:text-white">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-6">
        {stats?.map((stat) =>
          loading ? (
            <div
              className="glass-panel rounded-3xl p-5 lg:p-7 bg-white/40 dark:bg-slate-800/40"
              key={stat.id}
            >
            ...
            >
              <div className="flex items-start justify-between relative z-10">
                ...
                <div
                  className={`p-2.5 lg:p-4 rounded-xl lg:rounded-2xl backdrop-blur-sm flex-shrink-0 ${stat.type === "positive" ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500"}`}
                >
              ...
              <div className="mt-4 lg:mt-6 flex items-center gap-2 relative z-10">
                <span
                  className={`text-[10px] lg:text-xs font-black px-2 py-0.5 rounded-lg ${stat.type === "positive" ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-600"}`}
                >
                  {stat.change}
                </span>
        ...
        <div className="divide-y divide-black/5 dark:divide-white/5">
          {loading ? (
            Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="p-5 lg:p-6 flex items-center justify-between"
              >
                ...
                <div className="flex items-center gap-3 lg:gap-5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 truncate">
                      <span
                        className="text-[8px] lg:text-[10px] font-black px-2 py-0.5 rounded-lg bg-black/5 dark:bg-white/10 text-slate-500 dark:text-slate-400 uppercase tracking-widest"
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setModelValues({ ...model_values, title: e.target.value })
                  }
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setModelValues({ ...model_values, date: e.target.value })
                    }
